...
function getCanonicalRequest(now, host, path, query, headers, method, payload) {
  const canonicalRequest = method.toUpperCase() + '\n' +
    path + '\n' +
    query + '\n' +
    Object.keys(headers).map( header => `${header}:${headers[header]}`).join('\n').toLowerCase() + (Object.keys(headers).length ? '\n' : '') +
    'host:' + host + '\n' +
    'x-amz-date:' + getFormattedDate(now, 'long') + '\n' +
    '\n' +
    Object.keys(headers).join(';').toLowerCase() + (Object.keys(headers).length ? ';' :'') + 'host;x-amz-date' + '\n' +
...
function getAwsSignature(timeStamp, host, path, query, headers, method, region, service, accessKeyId, secretAccessKey, payload) {
  const canonicalRequest = getCanonicalRequest(timeStamp, host, path, query, headers, method, payload);
...
function getAuthHeader(timeStamp, host, path, query, headers, method, region, service, accessKeyId, secretAccessKey, payload) {
  const authHeader = 'AWS4-HMAC-SHA256 ' +
    'Credential=' + accessKeyId + '/' + getFormattedDate(timeStamp, 'short') + '/' + region + '/' + service + '/aws4_request, ' +
    'SignedHeaders=' + (headers['Content-Type'] || headers['content-type']  ? 'content-type:' :'') + 'host;x-amz-date, ' +
    'Signature=' + getAwsSignature(timeStamp, host, path, query, headers, method, region, service, accessKeyId, secretAccessKey, payload);
  return authHeader;
}

export const sign = function(opts, config) {
  const url = new URL(opts.url)
  const query = url.search.length && url.search[0] === '?' ? url.search.slice(1) : url.search
  const now = new Date();
  opts.headers = opts.headers || {}
  opts.headers.Authorization = getAuthHeader(now, url.hostname, url.pathname, query, opts.headers, opts.method, config.region, config.service, config.accessKeyId, config.secretAccessKey, opts.data);
